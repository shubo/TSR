...
batch_size_val = 100  # how many to process together
epochs_val = 25
...
def myModel():
    ...
    model.add((Conv2D(no_Of_Filters, kernel_size, input_shape=(imageDimensions[0], imageDimensions[1], 1),
                      activation='relu')))
    ...
    # COMPILE MODEL
    model.compile(optimizer=tensorflow.optimizers.Adam(learning_rate=0.005), loss='categorical_crossentropy', metrics=['accuracy'])
    return model

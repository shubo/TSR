...
plt.bar(range(0, num_classes-1), num_of_samples)
...
def preprocessing(img):
    ...
    img = img / 255 * 0.8 + 0.1  # TO NORMALIZE VALUES BETWEEN 0 AND 1 INSTEAD OF 0 TO 255
    return img


X_train = np.array(list(map(preprocessing, X_train)))  # TO IRRITATE AND PREPROCESS ALL IMAGES
...
def myModel():
    no_Of_Filters = 60
    kernel_size = (3, 3)  # THIS IS THE KERNEL THAT MOVE AROUND THE IMAGE TO GET THE FEATURES.
    # THIS WOULD REMOVE 2 PIXELS FROM EACH BORDER WHEN USING 32 32 IMAGE
    # size_of_Filter2 = (3, 3)
    size_of_pool = (2, 2)  # SCALE DOWN ALL FEATURE MAP TO GENERALIZE MORE, TO REDUCE OVERFITTING
    no_Of_Nodes = 500  # NO. OF NODES IN HIDDEN LAYERS
    model = Sequential()
    model.add((Conv2D(no_Of_Filters, kernel_size, input_shape=(imageDimesions[0], imageDimesions[1], 1),
                      activation='relu')))  # ADDING MORE CONVOLUTION LAYERS = LESS FEATURES BUT CAN CAUSE ACCURACY TO INCREASE
    model.add((Conv2D(no_Of_Filters, kernel_size, activation='relu')))
    model.add(MaxPooling2D(pool_size=size_of_pool))  # DOES NOT AFFECT THE DEPTH/NO OF FILTERS

    model.add((Conv2D(no_Of_Filters // 2, kernel_size, activation='relu')))
    # model.add((Conv2D(no_Of_Filters // 2, kernel_size, activation='relu')))
    ...
    # COMPILE MODEL
    model.compile(optimizer=tensorflow.optimizers.Adam(learning_rate=0.001), loss='categorical_crossentropy', metrics=['accuracy'])
    return model
...
history = model.fit(dataGen.flow(X_train, y_train, batch_size=batch_size_val),
                              steps_per_epoch=None, epochs=epochs_val,
